...
public class AmazonReportIngestor {

	private static final int HTTP_CREATED = 201;

	private static String generateCourseTakenRequest(String tomcatPrefix,
		...
		return HitApi.callService(tomcatPrefix
				+ ApiEndpoints.COURSE_TAKE_ENDPOINT + userId + "/" + courseId);
		...
		try {
			...
			for (int i = 1; i < lines.length; i++) {
				...
				if (splits.length != inputLength) {
					...
					String courseId = splits[6];
					if (courseId != null && !courseId.equals("")
							&& splits[7] != null && !splits[7].equals("")
							&& courseTitles.add(splits[7])) {
						totalCourses++;
						Course newCourse = new Course().withId(courseId);
						newCourse.withTitle(splits[7]);
						if (!(splits[13] == null && splits[13].equals("")))
							newCourse.withObjective(splits[13]);
						if (!(splits[9] == null && splits[9].equals("")))
							newCourse.withDescription(splits[9]);
					...
					if (updateDb)
						/*
						 * mySqlConnector.markRecommendationTaken(empId,
						 * courseId);
						 */
						System.out.println(generateCourseTakenRequest(
